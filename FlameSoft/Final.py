...
class MyForm(qt.QtWidgets.QMainWindow):
    ...
    def process(self):
        ...
        if self.ui.videoPath.text() == "" or self.ui.videoPath.text().endswith(('.avi', '.wmv')) != True:
            ...
            QMessageBox.warning(self, 'Error in File Type', 'Please check video input to be .avi or .wmv extension',
                                QMessageBox.Ok)
            ...
            try:
                ...
                if self.ui.previousBox.isChecked():
                    if self.Points == []:
                        QMessageBox.warning(self, 'Error Previous Image',
                                            'Please Uncheck the use previous image and choose image path to crop',
                                            QMessageBox.Ok)
                    else:
                        img = self.Points
                ...
                ##Video Process
                vidins = fsc.vid(self.ui.videoPath.text(), img, int(self.ui.filtersizeInput.text()),
                                 int(self.ui.pixelfilterInput.text()))
